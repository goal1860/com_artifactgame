...
//JLoader::register('K2HelperRoute', '/components/com_k2/helpers/route.php');
require_once JPATH_COMPONENT . '/../com_k2/helpers/route.php';
class ArtifactGameModelItem extends K2Model
{
    function getData()
    {
...
//		$item->params->merge($iparams);
        ...
        $query->select('*')
            ->from('#__artifactgame_card')
            ->where($db->quoteName('jid')." = ".$db->quote($item->id));
    ...
    function getTypeSummary() {
        ...
        $query = $db->getQuery(true);
        $query->select('count(*) as count, type');
        $query->from('#__artifactgame_card');
        $query->group('type');
        $query->order('type ASC');
        $db->setQuery($query);
        $typeList = $db->loadObjectList();
        $summary['type_list'] = $typeList;
        return $summary;
    }
}
